...
void *calc_compile(FIXED_STACK f)
{
    ...
    ITR(i, f.curr_item)
    {
        ...
        if (op->reduce){
            assert(op->stackchange==-1 && op->stackrequirement==2 && "INVALID REDUCE OPERATOR");
    ...
    char *exec_block = mmap(NULL, comp_size, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    ITR(i, f.curr_item)
    {
        ...
        {
            printf("ERROR: Instruction %ld requires stack to have atleast %ld item(s) but stack will have %ld\n", i, operation->stackrequirement, cstack);
...
int main()
{
    ...
    while (1)
    {
        ...
        if (input[0] == '\\')
        {
            ...
            {
                char fname = input[2];
                __int64_t (*func)(int, int, int) = calc_compile(to_ir(input + 3, strlen(input + 3)));
                if (func != NULL){
        ...
        __int64_t (*func)(int, int, int) = calc_compile(to_ir(input, strlen(input) + 3));
        if (func == NULL)
        {
            printf("ERROR: Failed to compile input\n");
        }
